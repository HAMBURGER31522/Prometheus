...
def render_report_image(run: Path) -> Path:
    run = run.resolve()
    output = run / "report.png"
    if output.is_file() and _has_current_width(output):
        return output
        ...
        try:
            ...
            def serve(route):
                ...
                if url.netloc == "report.local" and allowed and target.is_file():
                    route.fulfill(path=str(target), headers={"Content-Security-Policy": REPORT_CSP})
                else:
                    route.abort()
