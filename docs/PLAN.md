...
- Tauri 启动时先在 Rust 里取一个空闲端口（绑定 `127.0.0.1:0` 后立即释放），生成 32 字节随机 token，然后启动：
  ...
  并设置环境变量 `PYTHONUTF8=1`，`PATH` 前置 `runtime\node` 和 `runtime\ffmpeg`，Windows 进程创建标志带 `CREATE_NO_WINDOW`。
...
| POST | `/settings/test-model` | 用 `pi -p --no-tools` 发一个简短提示，返回是否成功以及模型能否看图 |
...
| POST | `/items/{id}/regenerate` | `{figures}`，复用已有转写 |
...
- **本地**（`transcribe/local_whisper.py`）：
  - 导入 ctranslate2 之前，对 `<数据目录>/runtime/cuda` 下每个 `nvidia/*/bin` 调用 `os.add_dll_directory`；
  ...
  - `language` 不指定（自动识别），`vad_filter=True`；
...
- **字幕**：`segments.json` 直接取 `asr.json` 的原始分段（不是规范化之后的单元）。SRT 格式：序号、`HH:MM:SS,mmm --> HH:MM:SS,mmm`、文本、空行，编码 UTF-8 无 BOM。TXT 格式：每行 `[HH:MM:SS] 文本`。
...
- 调用 vendor 的 `PiRunner`，参数经附录 A 的 V2 扩展：
  ...
  - `extra_prompt`：「本机为 Windows，命令工具是 PowerShell；运行 Python 用 `& $env:VIDEO_REPORT_PYTHON script.py`；脚本和中间文件只写在当前工作区。」开了配图时再追加：「另读 figures.md，按其中规则使用 frames/ 里的候选帧。」
  - `extra_files`：开了配图时加入 `report/overlays/figures.md`；
  - `timeout`：按第 2.1 节的公式；
  - 环境变量 `VIDEO_REPORT_PYTHON` 指向随包 python.exe，`PI_CODING_AGENT_DIR=<数据目录>/config/pi`。
...
- **分类**（`report/classify.py`）：`pi -p --no-tools --no-session --offline`。输入为已有分类名（最多 50 个）、报告标题、导语段、所有 `<h2>` 标题；要求只输出 JSON `{"category": "名称"}`；规则：优先选已有分类；新分类名为 2–8 个汉字的名词短语，不能用「其他 / 综合 / 杂项」这类名字。解析或校验失败时重试一次，仍然失败就归入「未分类」。

### 8.7 figures（配图）

- 模型能力：`settings/` 通过 `pi --offline --list-models` 的输出判断当前模型的 `images` 列是否为 `yes`（自定义提供商以设置里的勾选为准）。不能看图时跳过 `frames` 阶段，并在任务上提示。
...
- 调用：`pi -p --no-tools --no-session --offline`，要求只输出 Markdown。
- 格式要求：
  - 一行 `# 报告标题`；
...
- **知识库 / 思维导图 / 字幕**：都使用 `LibraryBrowser`，只有第三级的内容渲染器不同。
  ...
  - 第三级：顶部是面包屑「页签名 / 分类 / 报告标题」。
    - 知识库：`<iframe sandbox="allow-scripts" src="/api/items/{id}/report?token=...">`，占满宽度；
...
3. `uv export --package prometheus-backend --no-dev --no-hashes --no-emit-package playwright --no-emit-package mlx-whisper > build/requirements.txt`，然后用 runtime 里的 python 执行 `-m pip install --no-deps --target runtime\python\Lib\site-packages -r build\requirements.txt`，再安装 backend 和 vendor 两个本地包（同样 `--no-deps`）；
4. 删除 runtime 里的 `__pycache__`、`test/`、`tests/`；
5. `npm --prefix app run build`，然后 `npm --prefix app run tauri build -- --bundles nsis`；
6. 把安装包复制到 `E:\tools\Prometheus-Desktop\release\`，打印大小。
...
测试：导入 `prometheus.server` 之后，`sys.modules` 里不能出现 `playwright`（保证打包时去掉 playwright 不会出错）。
...
2. 引入 VRA：
   ...
   ```
3. 根目录 `pyproject.toml`（uv 工作区，dev 组包含 pytest==9.1.1、ruff==0.16.9，pytest 注册 `live` 标记）；`backend/pyproject.toml`（依赖为第 4 节列出的精确版本，以及工作区里的 `video-report-agent`）；运行 `uv lock` 和 `uv sync --all-packages`。
4. 后端：`server.py` 实现 `/api/health` 和 token 鉴权。先写 `tests/test_health.py`（包括不带 token 访问其他接口返回 401），看到它失败，再实现。
5. 前端：手工创建 `app/`（按第 4 节锁定版本），加一个 Vitest 冒烟测试；创建 `src-tauri`（按第 10 节写 `tauri.conf.json`，窗口按第 9.3 节）；`lint-design.mjs` 和 `tokens.css` 按第 9.2 节。
6. CI 工作流按第 11 节。
...
- `AsrRun` 转换（用伪造的 faster-whisper 分段）；设备选择逻辑（用桩函数模拟有 / 没有 CUDA 设备）；
- SRT / TXT 格式（包含超过 1 小时的时间戳）；
...
2. vendor 修改 V2（`PiRunner` 增加参数），并配测试：默认参数下生成的命令与修改前完全相同。
3. 先写的测试：工作区文件与 `input.json` 字段；生成的 Pi 命令里包含 `powershell`、不包含 `bash`、只提供 standard 模式；超时参数；定稿后的检查项（外部引用检测、`<h1>` 提取、简介占位符不残留）；分类的解析、校验和回退到「未分类」。
...
| 编号 | 内容 | 状态 |
|---|---|---|
| V1 | Windows 兼容：所有文本读写显式使用 UTF-8；`execution.py` 在 Windows 上使用 `CREATE_NEW_PROCESS_GROUP`，并用 `taskkill /T /F` 结束进程树；`pyproject.toml` 增加 `tzdata` 依赖；测试替身改成跨平台写法 | 未开始 |
| V2 | `PiRunner.__init__` 增加关键字参数 `tools`（默认 `"read,write,edit,bash"`，review 时追加 `inspect_report` 的逻辑不变）、`extra_prompt`（追加到用户提示末尾）、`extra_files`（复制进工作区）；默认值下行为与修改前完全一致 | 未开始 |
