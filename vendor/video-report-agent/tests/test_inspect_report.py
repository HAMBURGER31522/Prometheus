...
def test_browser_finds_defects_and_rechecks_current_report(tmp_path):
    (tmp_path / "transcript.md").write_text("[unit-1 | 0–1s] 内容", encoding="utf-8")
    ...
    (tmp_path / "report.html").write_text(
        '<html><body><p data-source-units="unit-1">修复后的正文</p></body></html>', encoding="utf-8"
    )
    second = inspect_report(tmp_path, "after")
    assert second["status"] == "checked"
    assert second["findings"] == []
    assert (tmp_path / "inspection/before/report.html").read_text(encoding="utf-8") != (
        tmp_path / "inspection/after/report.html").read_text(encoding="utf-8")
    assert len(second["screenshots"]) == 3


def test_browser_failure_is_not_a_pass(tmp_path, monkeypatch):
    (tmp_path / "report.html").write_text("<html><body>test</body></html>", encoding="utf-8")
    (tmp_path / "transcript.md").write_text("test", encoding="utf-8")
    ...
    inspection = json.loads((tmp_path / "inspection/final/result.json").read_text(encoding="utf-8"))
...
def test_inspection_only_fills_preview_not_agent_file(tmp_path):
    ...
    (tmp_path / "report.html").write_text(original, encoding="utf-8")
    (tmp_path / "transcript.md").write_text("[unit-1 | 0–1s] 正文", encoding="utf-8")
    (tmp_path / "download").mkdir()
    (tmp_path / "download/source.info.json").write_text('{"description":"简介"}', encoding="utf-8")
    result = inspect_report(tmp_path)
    assert result["status"] == "checked"
    assert (tmp_path / "report.html").read_text(encoding="utf-8") == original
    assert (tmp_path / "inspection/final/source.html").read_text(encoding="utf-8") == original
    preview = (tmp_path / "inspection/final/report.html").read_text(encoding="utf-8")
    ...
    (tmp_path / "report.html").write_text(preview, encoding="utf-8")
    final = inspect_report(tmp_path, "after-review")
    assert final["status"] == "checked"
    final_preview = (tmp_path / "inspection/after-review/report.html").read_text(encoding="utf-8")
